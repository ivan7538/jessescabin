...
abstract class Step {
	...
	protected function renderInvalid(){
		mphb_get_template_part( 'create-booking/errors', array(
	...
	protected function parseCheckInDate( $input ){
		...
		} else if ( \MPHB\Utils\DateUtils::calcNights( $today, $checkInDate ) < 0 ) {
	...
	protected function parseCheckOutDate( $input ){
		...
		} else if ( !is_null( $this->checkInDate ) && !MPHB()->getRulesChecker()->verify( $this->checkInDate, $checkOutDate ) ) {
