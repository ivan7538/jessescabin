...
class iCalSyncLogsMenuPage extends AbstractMenuPage
{
    ...
    public function __construct($name, $atts = array())
    {
        ...
        if (isset($_GET['queue'])) {
            $this->queue = sanitize_text_field($_GET['queue']);
        }
    ...
    public function render()
    {
        ...
        <div class="wrap mphb-sync-details-wrapper">
            ...
            <p><?php _e('Here you can see synchronization status of your external calendars.', 'motopress-hotel-booking'); ?></p>
            ...
            <?php if (is_null($this->queue)) {
                ...
                ?>
                <p>
                    <a href="<?php echo esc_url($syncAllUrl); ?>" class="button"><?php _e('Sync All External Calendars', 'motopress-hotel-booking'); ?></a>
                    <button class="button mphb-abort-process" <?php disabled(!$synchronizer->isInProgress()); ?>><?php _e('Abort Process', 'motopress-hotel-booking'); ?></button>
                    <button class="button mphb-clear-all"><?php _e('Delete All Logs', 'motopress-hotel-booking'); ?></button>
                </p>
            ...
            <?php } else {
                ...
                ?>
                <h2><?php echo !is_null($room) ? $room->getTitle() : ''; ?></h2>
            <?php } ?>

            <form id="<?php echo sanitize_key($this->listTable->get_plural() . '-filter'); ?>" method="POST" action="">
                <?php // We need to ensure that the form posts back to current page ?>
