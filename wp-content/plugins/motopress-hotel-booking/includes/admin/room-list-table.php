...
class RoomListTable extends \WP_List_Table {
	...
	public function column_cb( $item ){
		return '<input type="checkbox" name="ids[]" value="' . $item->getId() . '" />';
	}
	...
	public function column_title( $item ){
		...
		$actions = array(
			'edit'	 => sprintf( '<a href="' . $editUrl . '">%s</a>', __( 'Edit', 'motopress-hotel-booking' ) ),
//			'editinline'	 => sprintf( '<a href="#">%s</a>', __( 'Quick Edit', 'motopress-hotel-booking' ) ),
			'upload' => sprintf( '<a href="' . $uploadUrl . '">%s</a>', __( 'Import Calendar', 'motopress-hotel-booking' ) ),
			'sync'	 => sprintf( '<a href="' . $syncUrl . '">%s</a>', __( 'Sync External Calendars', 'motopress-hotel-booking' ) )
		);

		$roomType = MPHB()->getRoomTypeRepository()->findById( $item->getRoomTypeId() );

		$title = $item->getTitle();
		$title = '<strong><a href="' . $editUrl . '">' . ( !empty($title) ? $title : _x('(no title)', 'Placeholder for empty accommodation title','motopress-hotel-booking') ) . '</a></strong>';
		if ( $roomType ) {
