 ...
 */
class Aquentro_EDD_Updater_Admin {
	...
	function updater() {
		...
		if ( !class_exists( 'Aquentro_EDD_Updater' ) ) {
			// Load our custom theme updater
			include( dirname( __FILE__ ) . '/theme-updater-class.php' );
		}

		new Aquentro_EDD_Updater(
			array(
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), esc_html__( 'Error','aquentro' ) . $response->get_error_code() );
		}
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.','aquentro'  );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							esc_html__( 'Your license key expired on %s.' ,'aquentro'),
					...
					case 'revoked' :
						...
						$message = esc_html__( 'Your license key has been disabled.' ,'aquentro' );
						break;

					case 'missing' :

						$message = esc_html__( 'Invalid license.','aquentro'  );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = esc_html__( 'Your license is not active for this URL.' ,'aquentro' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.','aquentro'  ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = esc_html__( 'Your license key has reached its activation limit.','aquentro'  );
						break;
